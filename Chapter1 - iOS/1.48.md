...
### 1. Category 的真面目是 category_t 结构体
...
### 2. class 和 Category 的数据在编译期是分开的
...
### 3.  Category 数据是如何被加载到 Runtime 系统中和 class 的类对象、元类对象数据融合的
...
`memmove` 的效果是，将类原来的方法列表移动到第 addCount 个（addCount 为 category 方法列表长度位置，前面空出 addCount 个坑位，预留坑位给所有分类的方法）
...
Objective-C 的分类（Category）在**编译后**，底层会生成结构体 `struct category_t`，结构体**单独存储**分类的内容：

- 对象方法（instanceMethods）
- 类方法（classMethods）
- 实例属性（instanceProperties）
- 类属性（classProperties）
- 协议（protocols）

分类**不会**修改原始类的结构，也不会直接合并到原始类中。

**2. 运行时（Runtime 加载阶段）**

程序启动时，`_read_images` 会加载所有模块中的 `category_t`。

Runtime 会**自动将分类的数据，合并到对应的类对象 / 元类对象中**：

- 分类的**对象方法、实例属性、协议** → 合并到**类对象中**
- 分类的**类方法、类属性、协议** → 合并到**元类对象**中

**3. 方法排布规则（核心）**

在合并过程中，Runtime 会将：分类的方法、属性、协议全部插入到原始类的方法列表前方。最终形成顺序：
...
- Runtime 方法查找时**从前往后遍历**
- 优先找到分类方法，找到则不再调用
- 原始类方法依然存在，只是不会被调用



### 4. 为什么 Category 相关逻辑不能在编译期搞定，必须在 Runtime 运行时

有没有思考过一个问题，为什么 Category 里写的属性、协议、方法信息，不能在编译期处理好再写入 Mach-O，而必须设计为 App 启动后通过 Runtime 加载所有的 Category 然后合并到类的类对象、元类对象中？

**1.分类和类是分开编译的，编译器看不到彼此**。

类文件：`Person.m`，分类文件：`Person+Work.m`、`Person+Manage.m`，它们是完全独立的编译单元，编译器编译 `Person.m` 的时候：

根本不知道有没有分类、有多少分类、分类在哪里。编译器编译分类的时候：也不能回去修改已经编译好的类的 Mach-O 符号。

**2. Mach-O 目标文件是不可修改的**

类可能在：主程序、系统框架（UIKit）、动态库（.dylib/.framework）

**分类可以给任何类扩展，包括系统类！**

编译期怎么可能：

- 去修改 `UIKit.framework` 里的 `UIView` 的方法列表？
- 去修改动态库的 Mach-O？

**绝对不可能！**所以苹果只能统一设计成：**所有分类，无论在哪，一律运行时合并。**

**3. 链接期虽然能合并，但会破坏 Category 的核心特性**
...
OC 很多架构（插件化、模块化、热修复）**完全依赖 Category 动态性**。
...
#### 1. 为什么 `array()->lists + addedCount` 可以直接加 addedCount？

编译器底层会做计算，**DestinationAddress = 数组BaseAddress+  addedCount * sizeof(元素类型)**

等价于：`(array()->lists) + addedCount * sizeof(array()->lists[0])`

更加通用的解释是：**C 语言规定，指针加减数字，等价于指针地址加减数字乘以指针所指向数据类型的大小**

即 **指针 ± 数字 = 指针地址 ± (数字 × 指针指向的元素大小)**



#### 2. Runtime 新版代码修改了 Category 合并逻辑

Runtime 将 category 方法和类方法合并的时候，以前版本是  
...
看怎么定义可以？Category 可以定义属性。从源码角度来讲，查看分类的 category_t 结构体可以看到没有 `const ivar_list_t * ivars;` ，所以 category 声明属性底层只会生成 setter、getter 方法声明，没有实现。也就是分类可以声明属性 `@property`，但不能直接生成成员变量（Ivar）。需要程序员利用 runtime 关联属性自己实现

Category 底层结构体里没有 ivar 信息：
...
struct category_t {
    ...
    property_list_t *instanceProperties; // 属性声明
    // 没有：ivar_list_t *ivars !!!
};
```

在 Category 写属性 `@property (nonatomic, assign) int age;`，意味着

会做：

- 生成 **setter /getter 方法声明**
- 记录到 `property_list_t`（属性声明）

不会做：

- 不会生成成员变量 `_age`
- 不会生成 setter、getter 实现
...
因为 **runtime 查找方法是 从前向后遍历**：先查分类列表找到 `studentStudy` 直接返回，不往后查了！主类的同名方法永远没机会被调用

这就是 **Category 覆盖主类方法** 的**底层真相**！
...
void _read_images(header_info **hList, uint32_t hCount, int totalClasses, int unoptimizedTotalClasses)
{
    ...
    // 1. 首次初始化：全局表、TaggedPointer、non-pointer isa 等
    if (!doneOnce) {
        doneOnce = YES;
        // ... 省略初始化 ...
    }
    ...
    // 3. 重映射类引用、修复 @selector、协议处理 ...
    // ... 省略大量非核心代码 ...

    // 4.  realization 类（初始化 ro、rw、方法缓存等）
    for (EACH_HEADER) {
        classref_t *classlist = _getObjc2NonlazyClassList(hi, &count);
        for (i = 0; i < count; i++) {
            Class cls = remapClass(classlist[i]);
            realizeClass(cls);  // 核心：类实现
        }
    }

    // ======================================================
    // ✅ 核心：加载所有 Category（你最关心的部分）
    // ======================================================
    for (EACH_HEADER) {
        // 1. 获取当前镜像里的所有分类 category_t
        category_t **catlist = _getObjc2CategoryList(hi, &count);

        for (i = 0; i < count; i++) {
            category_t *cat = catlist[i];
            Class cls = remapClass(cat->cls);  // 找到分类所属的类
            ...
            // 处理【类方法 / 元类属性】
            // ------------------------------
            if (cat->classMethods || cat->protocols || cat->_classProperties) {
                addUnattachedCategoryForClass(cat, cls->ISA(), hi);
                if (cls->ISA()->isRealized()) {
                    remethodizeClass(cls->ISA()); // 参数为 class 的 isa，也就是处理分类中添加的类方法，到元类对象的类方法列表
                }
            }
        }
    }
...
可以看到：**remethodizeClass 方法调用了2次，分别传递了类对象、元类对象作为参数。它是负责方法重新排布的（将 Category 的方法、属性、协议重新组织到对应类上类对象、元类对象上去）**。内部会调用 attachCategories

- `remethodizeClass(cls)`：给【类对象】挂载【对象方法】 
- `remethodizeClass(cls->ISA())`：给【元类对象】挂载【类方法】
...
### 1. 为什么 +load 方法不是按照 Category 编译顺序倒序调用的？
...
1. +load 方法的收集时机：`load_images` → `prepare_load_methods`
   - 遍历 Mach-O 镜像
   - 找到所有**实现了 +load 的类**
   - 找到所有**实现了 +load 的分类**
   - 分别存入两个独立列表：
     - `loadable_classes`（类 +load）
     - `loadable_categories`（分类 +load）
2. `+load` 方法调用顺序：源码中先调用类的 +load（call_class_loads），再调用分类的 +load（call_category_loads）
3. 分类 +load 是【编译顺序】，不是倒序。分类 +load 是在 prepare_load_methods 阶段，按【镜像里的编译顺序】直接加入列表，不会逆序！
4. +load 不走消息机制，直接调用方法地址 IMP  `(load_method)(cls, @selector(load))`，不走 runtime 消息机制，所以不存在方法覆盖，类、分类的 +load 都会执行。



### 2. 为什么总是父类的 +load 比子类先执行？
...
![](https://raw.githubusercontent.com/FantasticLBP/knowledge-kit/master/assets/runtime-categoryattachLists.png)
  ...
  ### 1. 为什么给子类发消息，父类和子类的 +initialize 都会被调用？且父类的先调用
...
###  2. 为什么 `initialize`方法存在“覆盖”的情况？

有个现象：打印了 Student Category 的 `initialize`，却没有打印 Student 自己的 `initialize`，为什么？

查看 objc 源码发现调用 `initialize` 方法本质上就是通过 runtime 的 `objc_msgSend ` 发消息来实现的。也就是通过 isa 指针查看类对象或元类对象的方法列表中（方法列表中包含当前类各个 Category 的各个方法）查找方法实现。所以当 Runtime 找到方法列表中排列较前的 `initialize` 时，就不再继续查找方法实现了，也就出现了 `initialize` 被覆盖的情况了。
...
#### Demo1：继承关系加 Category，父类子类均实现 initialize 方法的顺序
...
其中 Student、Student+Good、Student+Bad、Teacher、Person 都实现了 initialize 方法，Person+Good 和 Person+Bad (编译顺序较后) 也实现了  initialize 方法.
...
- Student+Bad initialize
- Teacher initialize
```

为什么输出这样的信息？

`[Student alloc]` 等价于 `objc_msgSend([Student Class], @sel(alloc))` 然后 `objc_msgSend` 汇编实现里会调用 `lookUpImpOrForward`，`lookUpImpOrForward` 内部会调用 `initializeNonMetaClass`，其内部实现会判断是否存在父类，存在父类且父类没有调用过 `initialize`,则会递归先调用当前类的父类的 `initialize` 方法。递归结束则调用 `callInitialize` 方法去完成当前类的 `initialize` 

由于 initialize 方法本质就是 ``objc_msgSend([Person Class], @sel(initialize))`，但 Person 存在 2 个 Category，且 Person+Bad 编译顺序靠后，所以 Person+Bad 的 initialize 方法会被调用。

父类的 initialize 调用结束后，再调用子类 Student 的 initialize 方法，Student 也存在2个 Category，且 Student+Bad 的编译顺序靠后，所以 Student+Bad 的 initialize 方法先调用。

上述代码等价于

```objective-c
if (Student 类没有初始化过) {
  	// 有父类且父类没有 initilize 则递归先处理父类
    if (Student 父类（Person 父类存在，但存在分类，且 Person+Bad编译顺序靠后，运行时在加载分类时，会将分类的方法列表附加到类的方法列表前面，且后编译的分类会插入到更靠前的位置。因此方法查找时会优先命中后编译分类的实现，也就是 Person+Bad 的 initlize 被调用）存在 && 父类没有初始化) {
        objc_msgSend([Person class]，@selector(initializ)) // Person+Bad initialize
    }
  	// 处理完父类再处理子类
  	// Student 还未 initilize，但是消息发送时，由于存在分类，且 Student+Bad 靠后，Runtime 处理时逆序处理，所以 Student+Bad 影响消息
    objc_msgSend([Student class]，@selector(initializ))  // Student+Bad initialize  
}


if (Teacher 类没有初始化过) {
    // 有父类，但父类 Person 已经 initialize 过，if 里的逻辑进不去
    if (Teacher 父类（Person 父类存在，但存在分类，也就是 Person+Bad）存在 && 父类没有初始化) {
        objc_msgSend([Person class]，@selector(initializ)) 
    }
   // 处理完父类再处理子类
    objc_msgSend([Teacher class]，@selector(initializ)) // Teacher initialize
}
```

#### Demo2：继承关系加 Category，子类没有实现，父类实现 initialize 方法的顺序
...
共5个类，其中 Student、Teacher 都没有实现 initialize 方法。只有 Person 实现了、Person+Good 和 Person+Bad (编译顺序较后) 也实现了  initialize 方法.
...
可能你会有 “Demo2 为什么输出3次 Person+Bad initialize？不是 initialize 方法只有首次发消息才会被调用吗？” 这样的疑问。

但 initialize 方法保证的是类会被初始化1次，而不是方法。且子类没有实现则会继承父类的 initialize 方法。

`[Student alloc]` 等价于 `objc_msgSend([Student Class], @sel(alloc))` 然后 `objc_msgSend` 汇编实现里会调用 `lookUpImpOrForward`，`lookUpImpOrForward` 内部会调用 `initializeNonMetaClass`，其内部实现会判断是否存在父类，存在父类且父类没有调用过 `initialize`,则会递归先调用当前类的父类的 `initialize` 方法。递归结束则调用 `callInitialize` 方法去完成当前类的 `initialize` 。调用 initializeNonMetaClass 走的是 Runtime 消息机制，所以如果存在分类，则 Category 存在的同名方法会排在 class_rw_t 里 method_array_t 的前面，消息被响应后，后面的方法不会被响应，看上去就被“覆盖”了
...
if (Student 类没有初始化过) {
  	// 有父类且父类没有 initilize 则递归先处理父类
    if (Student 父类（Person 父类存在，但存在分类，且 Person+Bad编译顺序靠后，Runtime源码中会倒序访问分类，也就是 Person+Bad）存在 && 父类没有初始化) {
        objc_msgSend([Person class]，@selector(initializ)) // Person+Bad initialize
    }
  	// 处理完父类再处理子类
...
- 调用时刻：load 是 runtime 加载类、分类的时候调用的。initialize 是在类第一次接收消息的时候调用的。每个类只会 initialize 一次，但是父类的 initialize 可能会调用多次（比如子类第一次接收消息，但是子类内部没有实现 `+initialize`，由于消息机制，initialize 本质就是 objc_msgSend(class, @selector(initialize)),通过 class 找到类对象的方法列表，然后调用 initialize，但如果当前子类没有 initialize 方法则会通过 class 的 superclass 指针找到父类 isa 指针，找到类对象的方法列表去调用 initialize 方法，直到 NSObject 基类，父类如果实现了 `+initialize` 则会被调用。该父类至少调用2次）
...
## 七、为 Category 实现属性的 Setter 和 Getter
...
![](https://raw.githubusercontent.com/FantasticLBP/knowledge-kit/master/assets/AssociatedObjectPointerCrash.png)

查看上面的 Demo，会发现发生了 crash：

- person 对象出了花括号，引用计数变为0，内存释放成为野指针
- 调用关联对象 API，用 assign 去存储，本质不会对对象的内存做任何处理
- 花括号外，再用关联对象去访问的时候，拿出来已经是野指针，调用 NSLog 打印对象信息，系统为了打印成功，本质上会给该对象发送 retain 消息，所以堆栈信息中存在 `objc_retain` 这个 API
- 系统 objc_retain 访问野指针，会发生 EXC_BAD_ACCESS
